...
class Advisor:
			...
			if Query['callback_query']['data'] == 'order':
				...
				if fetchall:
					if fetchall[0][1] == 'advisor':
						return None
				await self.set_advisor_state(UserID)
				await self.new_advice_group(UserID)
				await self.end_advice_button(UserID)
				return
	...
	async def close_topic(self, UserID):
		...
		async with aiohttp.ClientSession() as session:
			await session.get(botdata.BASE_URL + 'deleteforumtopic?' +
				'chat_id=' + botdata.FORUM_ID + '&message_thread_id=' + str(TopicID))
		sql = 'UPDATE `BotUsers` SET topic = Default WHERE user_id=' + str(UserID) + ';'
		await aconexe(sql)
	...
	async def end_advice_button(self, UserID):
		...
		await buttons.send_markup(UserID, Markup, 'Чтобы закончить обращение, нажмите кнопку на клавиатуре телеграмма')